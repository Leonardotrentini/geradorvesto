...
export async function generateMannequin(
  ...
  try {
    ...
    const input = {
      ...
      num_inference_steps: 80, // Aumentado para melhor qualidade e aderência ao prompt profissional
      guidance_scale: 16.0, // Aumentado significativamente para forçar aderência ao prompt profissional (fiberglass, white glossy, proporcional)
    ...
    try {
      ...
      const fallbackInput = {
        prompt,
        negative_prompt: negativePrompt,
        num_inference_steps: 80, // Aumentado para melhor qualidade
        guidance_scale: 16.0, // Aumentado significativamente para forçar aderência ao prompt profissional
        width: 512,
        height: 768,
