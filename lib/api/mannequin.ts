...
export async function generateMannequin(
  ...
  try {
    ...
    const input = {
      ...
      num_inference_steps: 70, // Aumentado para melhor qualidade e aderência ao prompt
      guidance_scale: 15.0, // Aumentado significativamente para forçar aderência ao prompt (manequim branco puro, fundo preto puro)
    ...
    try {
      ...
      const fallbackInput = {
        prompt,
        negative_prompt: negativePrompt,
        num_inference_steps: 70, // Aumentado para melhor qualidade
        guidance_scale: 15.0, // Aumentado significativamente para forçar aderência ao prompt
        width: 512,
        height: 768,
