...
export function generateMannequinPrompt(config: PromptConfig): string {
  ...
  // Prompt profissional para manequim premium (baseado no briefing técnico)
  // CRÍTICO: Pipeline específica para manequim - não usar prompt genérico
  // Especificações técnicas:
  // - Fiberglass slim mannequin
  // - White glossy material (não matte)
  // - Anatomia proporcional
  // - Pose elegante fixa (legs slightly apart, arms relaxed)
  // - Studio lighting profissional
  // - Fundo preto sólido ou degradê
  // - Roupa aplicada com deformação natural
  return `full-body luxury fashion store mannequin wearing ${colorDesc} ${garmentType}. Slim fiberglass mannequin with smooth white glossy material, no facial features, proportional anatomy, elegant fashion pose, legs slightly apart, arms relaxed by the sides. Studio lighting with high-end shadows, solid pure black background, premium fashion photography, hyper-realistic details. The garment must match the mannequin shape perfectly, with natural fabric flow, realistic shadows and correct fabric deformation, no distortions, no artifacts, no texture loss, white on black, isolated, minimalist, clean, single mannequin only`
}
...
export function generateMannequinNegativePrompt(): string {
  // Negative prompt profissional (baseado no briefing técnico)
  // Bloqueia elementos que destroem a qualidade do manequim
  return `no human skin, no face, no wrinkles on body, no deformed limbs, no abstract shapes, no extra arms or legs, no blurred hands, no weird mannequin shapes, no melting textures, no cartoon style, no low-res, no background clutter, no unnatural shadows, no beige, no cream, no colored mannequin, no gray wall, no concrete, no texture, no patterns, no store interior, no background objects, no furniture, no decorations, no accessories, no glass cases, no retail environment, no boutique, no shop, no store, no environment, no walls, no floor, no lighting fixtures, no displays, no shelves, no racks, no hangers, no wooden hanger, no multiple mannequins, no realistic hands, no detailed hands, no body contours, no realistic features, no warm tones, no beige tones, no gray background, no textured background, no people, no faces, no realistic human face, no skin texture, no detailed facial features, no hair, no person, no blur, no watermark, no text, no logos, no extra limbs, no deformed mannequin, no golden mannequins, no stylized mannequins, no dynamic poses, no raised arms, no complex poses, no marble, no luxury details, no opulent atmosphere, no warm lighting, no sophisticated lighting, only pure white fiberglass mannequin, only solid pure black background, minimalist, clean, simple, isolated`
}
